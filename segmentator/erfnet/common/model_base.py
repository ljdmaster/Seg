...
from data.voc2012.data_processing import maybe_make_pardir, pickle2obj, obj2pickle, str2file #,load_batch_of_images
#from data.camvid.data_processing import maybe_make_pardir, pickle2obj, obj2pickle, str2file #,load_batch_of_images
        ...
        with tf.Session(graph=self.graph) as sess:
            ...
            try:
                ...
                for epoch in range(1, n_epochs+1):
                    ...
                    train_acc, train_loss = self.evaluate_in_session(data["X_train"][:1024], data["Y_train"][:1024], sess, batch_size=batch_size)
                    valid_acc, valid_loss = self.evaluate_in_session(data["X_valid"], data["Y_valid"], sess, batch_size=batch_size)
        ...
        with tf.Session(graph=self.graph) as sess:
            ...
            try:
                ...
                for epoch in range(1, n_epochs+1):
                    ...
                    # Evaluate on full train and validation sets after each epoch
                    train_iou, train_loss = self.evaluate_in_session(data["X_train"][:1024], data["Y_train"][:1024], sess, batch_size=batch_size)
